...
class LocalRegexTokenizer(val sparkTransformer: RegexTokenizer) extends LocalTransformer[RegexTokenizer] {
    ...
    localData.column(sparkTransformer.getInputCol) match {
      case Some(x) =>
        val newData = x.data.map { d =>
          ...
          val tokens = if (sparkTransformer.getGaps) re.split(str).toSeq else re.findAllIn(str).toSeq
...
object LocalRegexTokenizer extends LocalModel[RegexTokenizer] {
  override def load(metadata: Metadata, data: LocalData): RegexTokenizer = {
  ...
  override implicit def getTransformer(transformer: RegexTokenizer): LocalTransformer[RegexTokenizer] = new LocalRegexTokenizer(transformer)
}