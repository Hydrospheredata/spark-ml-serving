...
import io.hydrosphere.spark_ml_serving.regression.{LocalDecisionTreeRegressionModel, LocalGBTRegressor, LocalLinearRegressionModel, LocalRandomForestRegressionModel}
...
object TransformerConversions extends TransformerConverter {
  implicit def transformerToLocal(transformer: Transformer): LocalTransformable = {
    transformer match {
      case x : PipelineModel  => new LocalPipelineModel(x)(this)

      // Classification models
      case x: DecisionTreeClassificationModel  => new LocalDecisionTreeClassificationModel(x)
      case x: MultilayerPerceptronClassificationModel => new LocalMultilayerPerceptronClassificationModel(x)
