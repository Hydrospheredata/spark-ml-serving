...
object LocalGBTRegressor
  extends TreeModelLoader[GBTRegressionModel]
  with TypedTransformerConverter[GBTRegressionModel] {
  ...
  override def build(
    ...
    val parent = new GBTRegressor()
      ...
      .setMinInstancesPerNode(
        metadata.paramMap("minInstancesPerNode").asInstanceOf[Number].intValue()
      )
      .setCheckpointInterval(
        metadata.paramMap("checkpointInterval").asInstanceOf[Number].intValue()
      )
  ...
  override implicit def toLocal(
    transformer: GBTRegressionModel
  ) = new LocalGBTRegressor(transformer)
}
